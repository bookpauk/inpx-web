...
- полка "Избранное": скачанные книги попадают в нее автоматически, раздел доступен в OPDS
...
```js
{
    ...
    // полка "Избранное": каждая скачанная книга попадает в нее автоматически,
    // список хранится в <раб.дир>/favorites.json и переживает пересоздание поисковой БД
    // раздел доступен в OPDS по адресу /opds/favorites,
    // удаление книги с полки - /opds/favorites?del=<uid книги>
    // maxCount - сколько книг хранить, самые старые вытесняются
    "favorites": {
        "enabled": true,
        "autoAdd": true,
        "maxCount": 5000
